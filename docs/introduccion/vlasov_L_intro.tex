...
\begin{enumerate}
  \item con autogravedad, se deposita la masa en la malla radial con la función de peso
    $W_n$ (B-spline de orden $n$, ancho $\Delta r$), $\bar\rho(r_i)$;
  ...
  \item se interpola potencial y fuerza a cada partícula con el mismo $W_n$ (así no hay
    autofuerza), y se suman el fondo y el término centrífugo $L^2/r^3$ de su propio $L$;
...
\begin{aviso}
\textsc{detalle numérico: dos densidades.} El código también deposita una densidad
$\rho$ con la forma $S_n$ de ancho $\Delta r_c$, la celda de las partículas, solo como
salida. Cuando $\Delta r_c\neq\Delta r$ esa densidad no conserva la masa en la malla: en
un caso con $\Delta r_c=0.225$, $\Delta r=0.1$, $4\pi\int r^2\rho\,\dd r$ varió entre
$-3.9\%$ y $+4.8\%$. La que usa Poisson, $\bar\rho$, conserva la masa a $4\times10^{-5}$.
\end{aviso}
...
grupo & configuración \\
\midrule
fondos & \texttt{state=aa}, \texttt{Nrc=Npc=60}, \texttt{Nlc=1} ($L=2$), \texttt{a0=1e-4}, sin autogravedad, yoshida4, $t=200$; \texttt{BGtype}$\in$\{iso, isotrun, nfw, burkert\}, \texttt{courant}$=0.5,0.25,0.125$ ($\Delta t=0.025$, $0.0125$, $0.00625$) \\
...
$2.5\times10^{-7}$, $3.5\times10^{-7}$ y, con fondo \texttt{sphere}, $2.0\times10^{-7}$. La
...
\midrule
iso     & $3.16\times10^{-5}$ & $1.93\times10^{-6}$ & $1.20\times10^{-7}$ & $16.4$, $16.1$ \\
isotrun & $7.72\times10^{-7}$ & $4.85\times10^{-8}$ & $3.23\times10^{-9}$ & $15.9$, $15.0$ \\
nfw     & $4.64\times10^{-5}$ & $2.81\times10^{-6}$ & $1.74\times10^{-7}$ & $16.5$, $16.1$ \\
burkert & $8.14\times10^{-6}$ & $5.00\times10^{-7}$ & $3.11\times10^{-8}$ & $16.3$, $16.1$ \\
\bottomrule
\end{tabular}
\end{center}
...
$1.3\times10^{-7}$ en $1\leq r\leq15$, frente a $|\Phi_{\rm self}|\leq1.6\times10^{-3}$: es el
...
equilibrio, mapa numérico (potencial de equilibrio) & $1.3\times10^{-16}$ & $5.9\times10^{-6}$ & $1.6\times10^{-5}$ \\
...
coinciden ($2.9\times10^{-7}$ en $t=0$ con el instantáneo).
\end{aviso}
